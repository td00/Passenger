...
Thih module may not be the latest version of Squirrel.
This program developed using this version.
"""
...
db.queries(["some","thing"],[True,False],[False,False],[[],[1]])"""
	...
	def remove(self,row:int,col:int):
		"""Delete a singlecell on database. Remaining cells on the same row will justify left. Do not use on data science unless necesserity."""
		self-=self[row][col]
		return len(self)
